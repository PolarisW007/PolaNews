...
export async function GET(req: NextRequest) {
  try {
    ...
    const countResult = await queryOne(
      'SELECT COUNT(*) as total FROM user_article_states WHERE user_id = $1 AND is_starred = true',
      [user.id]
    ...
    const articles = rows.map(r => rowToArticle(r as Record<string, unknown>, { is_starred: true }));
...
export async function DELETE(req: NextRequest) {
  try {
    ...
    await execute(
      'UPDATE user_article_states SET is_starred = false WHERE user_id = $1 AND article_id = $2',
      [user.id, article_id]
    );
