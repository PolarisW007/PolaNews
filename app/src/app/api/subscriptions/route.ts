...
export async function GET(req: NextRequest) {
  try {
    ...
    const subscriptions: UserSubscription[] = rows.map((row: Record<string, unknown>) => ({
      id: row.id as string,
  ...
  try {
    ...
    await execute(
      `INSERT INTO user_subscriptions (id, user_id, feed_id, custom_label, group_name)
       VALUES ($1, $2, $3, $4, $5)`,
      [id, user.id, feed_id, custom_label || '', group_name || '']
    );
    ...
    const message = err instanceof Error ? err.message : '未知错误';
    if (message.includes('UNIQUE') || message.includes('user_id') || message.includes('feed_id') || message.includes('duplicate key')) {
      return NextResponse.json(
