...
/**
 * 将预置源写入数据库（INSERT OR IGNORE，基于 url 去重）
 */
export function seedPresetFeeds(db: Database.Database): void {
  ...
  const insertMany = db.transaction(() => {
    for (const feed of PRESET_FEEDS) {
      stmt.run(uuid(), feed.title, feed.url, feed.site_url, feed.language, feed.category);
    }
  });
