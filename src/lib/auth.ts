...
export function createUser(email: string, password: string, displayName: string): User {
  ...
  const preferences = JSON.stringify({ language: 'zh', theme: 'dark', digest_time: ['08:00'], categories: [] });

  db.prepare(
    'INSERT INTO users (id, email, password_hash, display_name, preferences) VALUES (?, ?, ?, ?, ?)'
  ).run(id, email, passwordHash, displayName, preferences);
