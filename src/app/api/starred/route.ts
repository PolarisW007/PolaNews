...
export async function GET(req: NextRequest) {
  try {
    ...
    const { total } = db
      .prepare(
        'SELECT COUNT(*) as total FROM user_article_states WHERE user_id = ? AND is_starred = 1'
      )
    ...
    const articles = rows.map(r => rowToArticle(r, { is_starred: true }));
...
export async function DELETE(req: NextRequest) {
  try {
    ...
    const db = getDb();
    db.prepare('UPDATE user_article_states SET is_starred = 0 WHERE user_id = ? AND article_id = ?')
