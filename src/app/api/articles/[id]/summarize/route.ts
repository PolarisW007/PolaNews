...
export async function POST(
  ...
  try {
    ...
    const db = getDb();
    const article = db.prepare('SELECT id, title, content, summary FROM articles WHERE id = ?').get(id) as {
      id: string;
      title: string;
      content: string;
      summary: string;
    ...
    db.prepare(
      `UPDATE articles SET ${aiSummaryCol} = ?, ai_key_points = ? WHERE id = ?`
    ).run(summary, JSON.stringify(key_points), id);
