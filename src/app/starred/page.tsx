...
export default function StarredPage() {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try {
      const data = await api.starred.list({ page, limit }) as StarredResult;
  ...
  const handleUnstar = async (articleId: string) => {
    try {
      await api.starred.remove(articleId);
      setArticles(prev => prev.filter(a => (a.article_id || a.id) !== articleId));
      setTotal(prev => prev - 1);
    } catch (e) { toast(e instanceof Error ? e.message : '取消收藏失败，请重试', 'error'); }
  };
